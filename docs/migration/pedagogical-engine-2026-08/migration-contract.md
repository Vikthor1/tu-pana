...
- **Classification:** **Preserve** (finalist UI + record shapes) + **Adapt** (orchestration: real per-call provider seam with profile-driven roles/mandates/prohibitions/synthesis order, all-or-nothing persistence, failure surfaces, usage accounting). **Deferred to live-model integration** (recorded in handoff.md): the structured anchor-validation kernel (`councilAnchorValid`, caps, corroboration recomputation) — it validates provider JSON the deterministic mock does not produce; `council.js` remains the cited translation source. The legacy `verifications`/`resolved` seam (unwired in legacy too) is likewise deferred.
